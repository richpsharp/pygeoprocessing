...
def greedy_pixel_pick_by_area(
        ...
        int ffi_buffer_size=2**11, int heap_buffer_size=2**28):
    ...
    cdef FastFileIteratorDoublePtr fast_file_iterator
    cdef vector[FastFileIteratorDoublePtr] fast_file_iterator_vector
    cdef int area_index = 0
    cdef long long i, n_elements = 0
    cdef double next_val = 0.0
    cdef double current_step = 0.0
    cdef double step_size, current_area
    result_list = []
    rm_dir_when_done = False

    working_sort_directory = tempfile.mkdtemp(dir=output_dir)

    file_index = 0
    nodata = pygeoprocessing.get_raster_info(
        base_value_raster_path_band[0])['nodata'][base_value_raster_path_band[1]-1]
    heapfile_list = []
    ...
    current_area = selected_area_report_list[area_index]
    step_size = 0
    if n_elements > 0:
        step_size = 100.0 / n_elements
    ...
    for i in range(n_elements):
        ...
        current_step = step_size * i
        next_val = fast_file_iterator_vector.front().next()
        if current_step >= current_area:
            result_list.append(next_val)
            area_index += 1
            if area_index >= len(selected_area_report_list):
                break
            current_area = selected_area_report_list[area_index]
        pop_heap(
            fast_file_iterator_vector.begin(),
    ...
    if rm_dir_when_done:
        shutil.rmtree(working_sort_directory)
    LOGGER.debug('here is selected_area_report_list: %s', str(result_list))
    shutil.rmtree(working_sort_directory)
    return result_list
