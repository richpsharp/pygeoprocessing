...
cdef int MANAGED_RASTER_N_BLOCKS = 2**9
...
def raster_optimization(
    ...
    Args:
        ...
        target_working_directory (str): path to a directory this function can
            ...
            `optimal_mask_{threshold_val}_{file_suffix}.tif`.
            If `target_suffix` is None it is not added to the target raster.
            The `threshold_val` is described below in `goal_met_cutoffs`.
        target_suffix (str): if not none, this suffix is added with a preceding
            '_' to the target filenames generated by this function.
        goal_met_cutoffs (list): list of sorted increasing order goal
            cutoffs to report for current optimization mask. When all objectives
            meet or exceed this value, a report will be recorded including
            a copy of the optimal mask raster in:
            'optimal_mask{file_suffix}_{threshold_val}.tif' and the
            final csv file report.
...
def greedy_pixel_pick_by_area(
    ...
    Args:
        ...
        output_directory (str): path to a directory this function can
            ...
            If `target_suffix` is None it is not added to the target raster.
            The `threshold_val` is described below in `goal_met_cutoffs`.
        area_step_report_list (list): list of sorted increasing order total
            area sizes in units expressed by ``area_raster_path_band``. When
            enough area is selcted to meet or exceed these values, a report
            will be recorded including a copy of the optimal mask raster in:
            'optimal_mask{file_suffix}_{area_amount}.tif' and the
            final csv file report.
        target_suffix (str): if not none, this suffix is added with a preceding
            '_' to the target filenames generated by this function.
    ...
    cdef int64t[:] index_data
    cdef FastFileIteratorIndexDoublePtr fast_file_iterator
    cdef FastFileIteratorIndexVectorPtr \
    ...
    for file_index, (offset_data, block_data) in enumerate(
        ...
        flat_indexes = (
            (yy.astype(numpy.int64)+numpy.int64(
                offset_data['yoff']))*n_cols +
            (xx.astype(numpy.int64)+numpy.int64(offset_data['xoff'])))[
                valid_mask].flatten()
        ...
        n_elements += buffer_data.size
        file_path = os.path.join(
            working_sort_directory, f'{file_index}.dat')
        index_path = os.path.join(
            working_sort_directory, f'index_{file_index}.dat')
        heapfile_list.append(file_path)
        ...
        # put on back and heapify
        deref(value_raster_fast_file_iterator_vector_ptr).push_back(
            fast_file_iterator)
        push_heap(
            deref(value_raster_fast_file_iterator_vector_ptr).begin(),
            deref(value_raster_fast_file_iterator_vector_ptr).end(),
            FastFileIteratorIndexCompare[double])

    LOGGER.debug('all sorted, starting optimization')
    if target_suffix is not None:
        target_suffix = f'_{target_suffix}'
    else:
        target_suffix = ''
    mask_raster_path = os.path.join(
        output_directory, f'working_mask{target_suffix}.tif')
    ...
    cdef _ManagedRaster area_managed_raster = _ManagedRaster(
        area_raster_path_band[0], area_raster_path_band[1], 0)
    ...
    with open(csv_path, 'w') as results_file:
        results_file.write(
            'sum of value,area selected (Ha),area cutoff (Ha)\n')

    cdef int area_target_index = 0
    cdef double area_so_far = 0.0
    cdef double value_sum = 0.0
    cdef double current_area_threshold = area_step_report_list[
        area_target_index]
    while True:
        # attempt to select a pixel
        active_index = (
            deref(value_raster_fast_file_iterator_vector_ptr).front().next())
        value_sum += deref(
            value_raster_fast_file_iterator_vector_ptr).front().get_last_val()
        # update the heap
        pop_heap(
            deref(value_raster_fast_file_iterator_vector_ptr).begin(),
            deref(value_raster_fast_file_iterator_vector_ptr).end(),
            FastFileIteratorIndexCompare[double])
        if deref(value_raster_fast_file_iterator_vector_ptr).back().size() > 0:
            push_heap(
                deref(value_raster_fast_file_iterator_vector_ptr).begin(),
                deref(value_raster_fast_file_iterator_vector_ptr).end(),
                FastFileIteratorIndexCompare[double])
            ...
            fast_file_iterator = deref(
                value_raster_fast_file_iterator_vector_ptr).back()
            del fast_file_iterator
            deref(value_raster_fast_file_iterator_vector_ptr).pop_back()
        ...
        count += 1
        mask_managed_raster.set(x, y, 1)
        area_so_far += area_managed_raster.get(x, y)

        pixels_set += 1
        # update all the pools
        if (area_so_far > current_area_threshold):
            # copy the mask to an intermediate value and save each
            # threshold value met so far
            LOGGER.debug(
                f'{area_so_far} met cutoff at {current_area_threshold}')

            target_step_raster_path = os.path.join(
                output_directory,
                'optimal_mask_'
                f'{area_step_report_list[area_target_index]}'
                f'{target_suffix}.tif')
            LOGGER.debug(f'writing to {target_step_raster_path}')
            mask_managed_raster.close()
            mask_raster = gdal.OpenEx(mask_raster_path, gdal.OF_RASTER)
            ...
            with open(csv_path, 'a') as results_file:
                results_file.write(
                    f'{value_sum},{area_so_far},'
                    f'{area_step_report_list[area_target_index]}\n')

            area_target_index += 1
            if area_target_index >= len(area_step_report_list):
                LOGGER.debug('met threshold max, quitting')
                break
            current_area_threshold = area_step_report_list[area_target_index]
            LOGGER.debug(
                f'next cutoff: {current_area_threshold}')
        if deref(value_raster_fast_file_iterator_vector_ptr).size() == 0:
            LOGGER.debug('no pixels left, quitting')
            break

    LOGGER.debug('met all requirements, freeing memory')
    # free all the iterator memory
    while deref(value_raster_fast_file_iterator_vector_ptr).size() > 0:
        fast_file_iterator = deref(
            value_raster_fast_file_iterator_vector_ptr).back()
        del fast_file_iterator
        deref(value_raster_fast_file_iterator_vector_ptr).pop_back()
    del value_raster_fast_file_iterator_vector_ptr
    ...
    try:
        ...
        LOGGER.warning(f'unable to remove {working_sort_directory}')
